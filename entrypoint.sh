...
for disabled in $2; do
	if [ -x "/commands/$disabled" ]; then
		echo "disable command: $disabled" >/dev/stderr
		rm "/commands/$disabled"
	else
		echo "cannot delete command: $disabled" >/dev/stderr
	fi
done
...
if [[ $text == ${prefix}* ]]; then
  ...
  cmdName=${cmdName//\//"\\\\/"}
  echo "cmd test: $text" > /dev/stderr
  args="$(echo "$text" | cut -d' ' -f2-)"|| ( echo "err getting args" && exit $?)
  echo "cmd args: $args" > /dev/stderr
